...
class Table(Canvas):
    ...
    def setRowColors(self, rows=None, clr=None, cols=None,ordre=None):
        ...
        if ordre == None :
            file = os.path.abspath("Colors\\COLORS")
            with codecs.open(file, "a", "utf-8") as filout:
                filout.write(
                    'COLORSLINE' + self.__dict__['regle'] + str(self.__dict__['num']) + "                   " + str(rows) +"                   "
                    + clr + "\n")
    ...
    def saveAs(self, filename=None):
        ...
        filename = os.path.abspath("SchemaComptable") + "\\shmComp_" + self.__dict__['regle'] +'_'+ str(self.__dict__['num']) + '.xls'
        if filename == None:
